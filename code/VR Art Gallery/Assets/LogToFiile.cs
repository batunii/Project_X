...
public class CloudLogForwarder : MonoBehaviour
{
    ...
    async void Awake()
    {
        ...
        try
        {
            ...
            // IMPORTANT: Must authenticate before calling Cloud Code
            if (!AuthenticationService.Instance.IsSignedIn)
            {
                await AuthenticationService.Instance.SignInAnonymouslyAsync();
                Debug.Log("CloudLogForwarder: Signed in anonymously.");
            }
        ...
        {
            Debug.LogError($"CloudLogForwarder: Failed to initialize: {e.Message}");
        }
    }

    async void HandleLog(string logString, string stackTrace, LogType type)
    {
        // Don't send our own logs to avoid recursion
